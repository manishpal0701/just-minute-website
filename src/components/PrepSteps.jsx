...
import { Droplet, PlusCircle, RefreshCw, Snowflake, Sparkles, CheckCircle2, ArrowDown, Zap, Heart } from 'lucide-react';
...
export default function PrepSteps({ activeFlavour }) {
  ...
  useGSAP(() => {
    ...
    // 2. Clear stale inline styles from target animation elements
    const stageElements = containerRef.current.querySelectorAll(
      '.demo-water-bottle, .demo-water-stream, .demo-pack-sachet, .demo-powder-stream, .demo-stir-spoon, .demo-vortex-ring, .demo-ice-1, .demo-ice-2, .demo-mint, .demo-splash-wave, .demo-final-aura, .demo-sparkle-burst, .demo-final-badge, .demo-outro-bar, .demo-glass-liquid, .step-node-01, .step-node-02, .step-node-03, .step-node-04, .step-node-05, .demo-arrow-1, .demo-arrow-2, .demo-arrow-3, .demo-arrow-4'
    );
    gsap.set(stageElements, { clearProps: 'transform,opacity,height,scale,rotate,x,y' });

    // 3. Set initial GSAP states for all animated stage elements
    ...
    // 4. Master ScrollTrigger Timeline with Pinning & Scrubbing
    ...
    // ------------------------------------------------------------------------
    // PHASE 1: 01 POUR (0.0 -> 0.20 Progress)
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // PHASE 2: 02 ADD POWDER (0.20 -> 0.40 Progress)
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // PHASE 3: 03 STIR (0.40 -> 0.60 Progress)
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // PHASE 4: 04 CHILL (0.60 -> 0.80 Progress)
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // PHASE 5: 05 SERVE & OUTRO TRANSITION (0.80 -> 1.0 Progress)
    // ------------------------------------------------------------------------
    ...
    // 5. Refresh ScrollTrigger after mounting timeline to calculate accurate page positions
  ...
  return (
    <section 
      ref={containerRef} 
      id="prep" 
      className="relative min-h-screen w-full flex flex-col justify-between py-6 px-4 sm:px-8 overflow-hidden z-10 shadow-sm transition-colors duration-500"
      style={{
        backgroundColor: activeFlavour?.colors?.background || '#FFF6DB',
        color: activeFlavour?.colors?.dark || '#3E1A00'
      }}
    >
      <div className="max-w-7xl mx-auto w-full relative z-10 flex flex-col justify-between min-h-[92vh]">
        
        {/* Top Bar Header & Progress Tracker */}
        <div 
          className="flex flex-col md:flex-row items-center justify-between gap-4 pb-3 border-b-2 shrink-0"
          style={{ borderColor: `${activeFlavour?.colors?.dark || '#3E1A00'}20` }}
        >
          <div className="text-left">
            <div 
              className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full font-sheetal font-black text-xs uppercase tracking-wider shadow-sm border-2 text-white"
              style={{
            ...
            <h2 
              className="font-sheetal font-black text-3xl sm:text-5xl tracking-tight mt-1.5"
          ...
          <div 
            className="flex items-center gap-3 bg-white border-3 px-5 py-2.5 rounded-full shadow-md shrink-0"
          ...
          >
            ...
            <span 
              className="font-sheetal font-black text-base sm:text-lg"
              style={{ color: activeFlavour?.colors?.dark || '#3E1A00' }}
            >
        ...
        {/* Main Demonstration Stage: 2-Column Grid */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center my-auto py-3 grow">
          
          {/* Left Column: 01 -> 02 -> 03 -> 04 -> 05 Vertical Step Timeline */}
          <div className="lg:col-span-5 flex flex-col space-y-2.5 sm:space-y-3 relative">
          ...
          {/* Right Column: Dynamic Interactive Glass Canvas */}
          <div className="lg:col-span-7 flex flex-col items-center justify-center relative min-h-[420px] sm:min-h-[480px]">
            ...
            <div 
              className="demo-final-aura absolute w-96 h-96 rounded-full blur-3xl pointer-events-none transition-opacity duration-500 opacity-20"
              style={{ backgroundColor: liquidColor }} 
            />

            {/* Stage Showcase Area */}
            <div className="relative w-full max-w-lg h-[440px] flex items-center justify-center">

              {/* 1. Water Bottle / Pitcher (Step 01 Element) */}
              <div className="demo-water-bottle absolute -top-8 right-8 z-30 pointer-events-none">
                <div 
                  className="w-24 h-48 bg-white rounded-3xl border-3 p-2 flex flex-col items-center justify-center shadow-md"
                  style={{ borderColor: activeFlavour?.colors?.dark || '#3E1A00' }}
                >
                  <div className="w-12 h-6 bg-[#7BE0E8] rounded-t-lg mb-2" />
                  <Droplet className="w-10 h-10 text-[#7BE0E8] animate-bounce" />
                  <span className="text-[10px] font-sheetal font-black uppercase mt-2" style={{ color: activeFlavour?.colors?.dark || '#3E1A00' }}>Mineral Water</span>
                </div>
              </div>

              {/* Water Stream (Step 01 Animation) */}
              <div 
                className="demo-water-stream absolute top-16 right-36 w-4 bg-[#7BE0E8] rounded-full z-20 pointer-events-none border"
                style={{ borderColor: activeFlavour?.colors?.dark || '#3E1A00' }} 
              />

              {/* 2. Product Sachet Pack (Step 02 Element) */}
              <div className="demo-pack-sachet absolute -top-10 right-10 z-30 pointer-events-none">
                <img 
                  key={activeFlavour?.id || 'mango'}
                  src={activeFlavour?.assets?.box1l || '/assets/products/box-1l-mango.png'} 
                  alt={flavourName}
                  onLoad={handleImageLoad}
                  className="w-32 h-44 object-contain filter drop-shadow-[0_10px_15px_rgba(0,0,0,0.12)]"
                />
              </div>

              {/* Powder Stream (Step 02 Animation) */}
              <div 
                className="demo-powder-stream absolute top-20 right-32 w-5 rounded-full z-20 pointer-events-none border"
                style={{ backgroundColor: liquidColor, borderColor: activeFlavour?.colors?.dark || '#3E1A00' }}
              />

              {/* 3. Stirring Spoon (Step 03 Element) */}
              <div className="demo-stir-spoon absolute -top-24 left-1/2 -translate-x-1/2 z-30 pointer-events-none">
                <div 
                  className="w-4 h-64 bg-slate-300 rounded-full border-2 shadow-md flex flex-col items-center justify-end pb-2"
                  style={{ borderColor: activeFlavour?.colors?.dark || '#3E1A00' }}
                >
                  <div className="w-8 h-10 rounded-full bg-slate-200 border shadow-inner" style={{ borderColor: activeFlavour?.colors?.dark || '#3E1A00' }} />
                </div>
              </div>

              {/* Vortex Swirl Ring inside Glass (Step 03) */}
              <div 
                className="demo-vortex-ring absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-36 h-36 border-4 border-dashed rounded-full z-20 pointer-events-none"
                style={{ borderColor: activeFlavour?.colors?.dark || '#3E1A00' }} 
              />

              {/* 4. Ice Cubes & Mint Leaf (Step 04 Elements) */}
              <div className="demo-ice-1 absolute top-0 left-1/3 z-20 pointer-events-none">
                <div 
                  className="w-10 h-10 bg-white rounded-xl border-2 shadow-md flex items-center justify-center rotate-12"
                  style={{ borderColor: activeFlavour?.colors?.dark || '#3E1A00' }}
                >
                  <Snowflake className="w-5 h-5 text-[#7BE0E8]" />
                </div>
              </div>

              <div className="demo-ice-2 absolute top-0 right-1/3 z-20 pointer-events-none">
                <div 
                  className="w-11 h-11 bg-white rounded-xl border-2 shadow-md flex items-center justify-center -rotate-12"
                  style={{ borderColor: activeFlavour?.colors?.dark || '#3E1A00' }}
                >
                  <Snowflake className="w-6 h-6 text-[#7BE0E8]" />
                </div>
              </div>

              <div className="demo-mint absolute top-0 left-1/2 z-20 pointer-events-none">
                <div 
                  className="w-8 h-8 bg-[#72C96B] rounded-full border-2 flex items-center justify-center shadow-md"
                  style={{ borderColor: activeFlavour?.colors?.dark || '#3E1A00' }}
                >
                  🍃
                </div>
              ...
              {/* Splash Wave effect */}
              <div 
                className="demo-splash-wave absolute top-1/3 left-1/2 -translate-x-1/2 w-48 h-8 rounded-full bg-white/60 blur-sm z-20 pointer-events-none border"
                style={{ borderColor: activeFlavour?.colors?.dark || '#3E1A00' }}
              />

              {/* Central 3D Drink Glass Container */}
              <div 
                className="relative w-56 h-80 rounded-b-3xl border-4 bg-white shadow-lg overflow-hidden flex flex-col justify-end p-2 z-10"
                style={{ borderColor: activeFlavour?.colors?.dark || '#3E1A00' }}
              >
                
                {/* Glass Rim */}
                <div 
                  className="absolute top-0 left-0 right-0 h-4 border-b-2 bg-white/60"
                  style={{ borderColor: activeFlavour?.colors?.dark || '#3E1A00' }}
                />
                ...
                <div className="absolute inset-y-0 left-2 w-4 border-l-2 pointer-events-none opacity-20" style={{ borderColor: activeFlavour?.colors?.dark || '#3E1A00' }} />
              </div>
              ...
              <div className="demo-sparkle-burst absolute inset-0 pointer-events-none flex items-center justify-center z-30">
                <div className="absolute -top-6 left-6 font-bubble text-4xl animate-bounce" style={{ color: activeFlavour?.colors?.primary || '#FF9800' }}>
                  ✦
                </div>
                <div className="absolute top-10 right-4 font-bubble text-5xl animate-bounce" style={{ color: activeFlavour?.colors?.primary || '#FF9800' }}>
                  ★
                </div>
              </div>

              <div className="demo-final-badge absolute -bottom-4 z-40">
                <div 
                  className="px-7 py-3.5 border-3 shadow-md flex items-center gap-3 rounded-2xl bg-white"
                  style={{ borderColor: activeFlavour?.colors?.dark || '#3E1A00' }}
                >
                  <Heart className="w-6 h-6 fill-current animate-pulse" style={{ color: activeFlavour?.colors?.primary || '#FF9800' }} />
                  <div className="text-left">
                    <div className="font-bubble text-xl leading-none" style={{ color: activeFlavour?.colors?.dark || '#3E1A00' }}>
                      Ready to Enjoy in Under 60 Seconds!
                    </div>
                    <div className="font-sheetal font-black text-xs uppercase tracking-wider mt-1 opacity-80" style={{ color: activeFlavour?.colors?.dark || '#3E1A00' }}>
        ...
        <div 
          className="demo-outro-bar pt-3 border-t-2 flex items-center justify-between text-xs font-sheetal font-black uppercase tracking-wider shrink-0"
          style={{ borderColor: `${activeFlavour?.colors?.dark || '#3E1A00'}20`, color: activeFlavour?.colors?.dark || '#3E1A00' }}
        >
          <span className="flex items-center gap-1.5">
            <Sparkles className="w-3.5 h-3.5 animate-pulse" style={{ color: activeFlavour?.colors?.primary || '#FF9800' }} />
            <span>Preparation Complete • Transitioning to Gourmet Mixology Studio</span>
          </span>
